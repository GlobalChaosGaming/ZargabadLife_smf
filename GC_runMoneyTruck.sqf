...
//start mission loop
"server globalchat ""The Money Truck has spawned! Civs: kill the driver to stop it and steal the money. Cops: your bonuses are inside, make sure it gets to the base!"";" call broadcast;
while {true} do {
	...
	if (!alive convoysoldier and !_added) then {
		...
		"if (isciv) then {server sidechat ""The Money Truck driver is dead. Steal the police bonuses."";};" call broadcast;
		"if (iscop) then {server sidechat ""The Money Truck driver is dead. Get in his truck and drive it to the Cop Base"";};" call broadcast;
	...
	if (convoytruck distance getmarkerpos "policebase" < 150) exitwith
		{
		"if (iscop) then {Kontostand = (Kontostand + govconvoybonus); player sidechat format[""you received $%1 for the successfully escorting the Money Truck"", govconvoybonus];};" call broadcast;
		_sidewon = "Cops";
		deletevehicle convoytruck;
	...
	if (GETDAMMAGE convoytruck >= 0.8) exitwith {
		"server globalchat ""The Money Truck has been destroyed the cop bonuses have burned"";" call broadcast;
		_sidewon = "Neither";
