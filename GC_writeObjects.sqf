...
//mission values
_date = Date;
"gc" callExtension "D|__SERVER__|GC_Mission";
"gc" callExtension format["W|%1|%2|%3|%4","__SERVER__","GC_Mission","DateStamp",_date];
if (count gangsarray > 0) then {
	"gc" callExtension format["W|%1|%2|%3|%4","__SERVER__","GC_Mission","gangsarray",gangsarray];
	{
		_gang = _x getvariable "control";
		if (!isnil "_gang") then {
			"gc" callExtension format["W|%1|%2|%3|%4","__SERVER__","GC_Mission",_x,_gang];
		};
	} foreach gangareas;
};
...
"gc" callExtension "D|__SERVER__|GC_Objects";
_counter = 0;
{
	...
	if (!isnil "_droparray") then {
		if (count _droparray > 1) then {
			...
			if (_itemType != "drug" and _itemType != "drink" and _itemType != "food" and _itemType != "ressource" and _itemType != "bomb" and _itemType != "Ring" and _itemType != "static") then {
				...
				_arr =[_pos,_dir,_class,_droparray];
				"gc" callExtension format["W|%1|%2|%3|%4","__SERVER__","GC_Objects",_counter,_arr];
				_counter = _counter + 1;
			};
		};
	};
} foreach (worldCenterPosition nearObjects ["Suitcase", 8000]);
diag_log text format["Wrote %1 dropped suitcases",_counter];
...
_unsorted = [];
{
	...
	_distance = _x select 1;
	{
		...
		if (!isnil "_created" and _damage < 0.98) then {
			...
			_arr = [_pos,_dir,_damage,_class,_name,_weps,_mags,_trunk,_nitro,_tuning,_caralarm,_customhorn,_radar,_mount,_skin,_bunny,_fuel,_on];
			_hasmount = 1;
			if (_mount != "") then {
				_hasmount = 0;
			};
...
"gc" callExtension "D|__SERVER__|GC_Vehicles";
_sorted = [_unsorted,0] call CBA_fnc_sortNestedArray;
_counter = 0;
{
	_arr = _x select 1;
	"gc" callExtension format["W|%1|%2|%3|%4","__SERVER__","GC_Vehicles",_counter,_arr];
	_counter = _counter + 1;
} foreach _sorted;
...
"gc" callExtension format["W|%1|%2|%3|%4","__SERVER__","GC_Server","TLS",time_last_saved];
